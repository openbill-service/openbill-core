...
* Таблица `OPENBILL_HOLDS` - блокировка средств на счёте. Подробнее — в разделе «Блокировка средств (Holds)».
...
Holds (холды) — механизм временной заморозки средств на счёте. Типичное применение:

* **Бронирование при оплате** — заблокировать сумму заказа до подтверждения доставки
* **Авторизация банковской карты** — hold на сумму покупки перед capture
* **Депозит** — заморозить залоговую сумму на время аренды

Заблокированные средства остаются на счёте, но недоступны для transfers. Баланс счёта разделяется на две части:

* `amount_value` — доступные средства (можно тратить)
* `hold_value` — замороженные средства (нельзя тратить)

Реальный баланс счёта = `amount_value + hold_value`.
...
-- Результат: amount_value = 300, hold_value = 200
```

## Разблокировка

Вставляется запись с отрицательной суммой и ссылкой на исходный hold через `hold_key`:
...
-- Результат: amount_value = 500, hold_value = 0
```

Разблокировка может быть частичной — можно разблокировать 100 из 200, а остальное позже.

## Capture (списание из холда)

Типичный платёжный сценарий: заблокировать средства → подтвердить операцию → списать.
Capture делается в одной SQL-транзакции — разблокировка + transfer:
...
Если capture не состоялся (например, заказ отменён) — достаточно просто разблокировать средства.

## Срок жизни холдов

Openbill не управляет сроком жизни холдов — это ответственность приложения. Приложение должно самостоятельно отслеживать устаревшие холды (например, по полю `created_at`) и разблокировать их.
...
* Конкурентные блокировки одного счёта безопасны (row-level locking)
