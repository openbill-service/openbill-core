...
SELECT
  'Donations: Donor -> CampaignEscrow',
  fc.id,
  tc.id,
  true
FROM openbill_categories fc, openbill_categories tc
WHERE fc.name = 'Donor' AND tc.name = 'CampaignEscrow'
...
INSERT INTO openbill_policies (name, from_category_id, to_category_id, allow_reverse)
SELECT
  'Donations: CampaignEscrow -> Beneficiary',
  fc.id,
  tc.id,
  false
FROM openbill_categories fc, openbill_categories tc
WHERE fc.name = 'CampaignEscrow' AND tc.name = 'Beneficiary'
ON CONFLICT (name) DO UPDATE
SET from_category_id = EXCLUDED.from_category_id,
    to_category_id = EXCLUDED.to_category_id,
    allow_reverse = EXCLUDED.allow_reverse;
...
INSERT INTO openbill_policies (name, from_category_id, to_category_id, allow_reverse)
SELECT
  'Donations: CampaignEscrow -> PlatformFee',
  fc.id,
  tc.id,
  false
FROM openbill_categories fc, openbill_categories tc
WHERE fc.name = 'CampaignEscrow' AND tc.name = 'PlatformFee'
ON CONFLICT (name) DO UPDATE
SET from_category_id = EXCLUDED.from_category_id,
    to_category_id = EXCLUDED.to_category_id,
    allow_reverse = EXCLUDED.allow_reverse;
