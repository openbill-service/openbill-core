...
PGDATABASE="$PGDATABASE" PGUSER="$PGUSER" PGHOST="$PGHOST" PGPASSWORD="$PGPASSWORD" psql --set ON_ERROR_STOP=1 -f "$SCRIPT_DIR/categories-and-policies.sql" >/dev/null

printf '[%s] run operations.sql\n' "$EXAMPLE_NAME"
PGDATABASE="$PGDATABASE" PGUSER="$PGUSER" PGHOST="$PGHOST" PGPASSWORD="$PGPASSWORD" psql --set ON_ERROR_STOP=1 -f "$SCRIPT_DIR/operations.sql" >/dev/null
