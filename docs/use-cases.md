...
| Страхование | Сбор премий, резервирование и выплаты по страховым случаям | `PremiumInflow`, `InsuranceReserve`, `ClaimsPayout`, `InsuranceFee` | `PremiumInflow -> InsuranceReserve`, `InsuranceReserve -> ClaimsPayout`, `InsuranceReserve -> InsuranceFee` |
| Зарплатный проект | Массовые выплаты сотрудникам через payroll clearing | `EmployerFunding`, `PayrollClearing`, `EmployeeAccount`, `TaxAccount` | `EmployerFunding -> PayrollClearing`, `PayrollClearing -> EmployeeAccount`, `PayrollClearing -> TaxAccount` |
| Кредит / BNPL | Выдача займа, погашение тела долга и процентов | `LenderFund`, `BorrowerAccount`, `PrincipalRepayment`, `InterestIncome`, `PenaltyIncome` | `LenderFund -> BorrowerAccount`, `BorrowerAccount -> PrincipalRepayment`, `BorrowerAccount -> InterestIncome`, `BorrowerAccount -> PenaltyIncome` |
| Карточный эмитент | Авторизации, клиринг мерчантов, fee и chargeback-контур | `CardholderAccount`, `CardAuthHold`, `MerchantSettlement`, `CardFeeIncome`, `ChargebackReserve` | `CardholderAccount -> CardAuthHold`, `CardAuthHold -> MerchantSettlement`, `CardholderAccount -> CardFeeIncome`, `CardAuthHold -> ChargebackReserve` |
| Ремиттансы | Трансграничный перевод через escrow с выделенной FX-комиссией | `SenderSource`, `RemitEscrow`, `RecipientPayout`, `FXFee` | `SenderSource -> RemitEscrow`, `RemitEscrow -> RecipientPayout`, `RemitEscrow -> FXFee` |
| Travel / Booking | Предоплата клиента с расчётом с поставщиком и комиссией OTA | `Traveler`, `BookingEscrow`, `SupplierPayout`, `OTAFee` | `Traveler -> BookingEscrow`, `BookingEscrow -> SupplierPayout`, `BookingEscrow -> OTAFee` |
| Реферальная программа | Начисление реферальных бонусов и последующая выплата | `Revenue`, `ReferralAccrual`, `PartnerWallet`, `ReferralReversal` | `Revenue -> ReferralAccrual`, `ReferralAccrual -> PartnerWallet`, `ReferralAccrual -> ReferralReversal` |
| Рекламная сеть | Депозиты рекламодателей и выплаты паблишерам через campaign escrow | `AdvertiserDeposit`, `CampaignEscrow`, `PublisherPayout`, `NetworkFee` | `AdvertiserDeposit -> CampaignEscrow`, `CampaignEscrow -> PublisherPayout`, `CampaignEscrow -> NetworkFee` |
| Телеком prepaid | Пополнение баланса абонента и списание за услуги | `TopupSource`, `SubscriberWallet`, `ServiceConsumption`, `TelecomFee` | `TopupSource -> SubscriberWallet`, `SubscriberWallet -> ServiceConsumption`, `SubscriberWallet -> TelecomFee` |
| Лояльность / Бонусы | Начисление бонусов, трата и учёт сгорания | `BonusLiability`, `UserBonusWallet`, `RedemptionSink`, `ExpiredBonusIncome` | `BonusLiability -> UserBonusWallet`, `UserBonusWallet -> RedemptionSink`, `BonusLiability -> ExpiredBonusIncome` |
