...
| `amount` | `numeric(36,18)` | да | - | Сумма удержания (> 0 — блокировка, < 0 — разблокировка) |
| `currency` | `varchar(8)` | да | `USD` | Валюта |
...
- Блокировка (`amount > 0`): `hold_key` должен быть `NULL`.
- Разблокировка (`amount < 0`): `hold_key` обязан ссылаться на `idempotency_key` существующего удержания.
- Это гарантируется CHECK-ограничением: `(amount < 0 AND hold_key IS NOT NULL) OR (amount > 0 AND hold_key IS NULL)`.
...
SET balance     = balance     - NEW.amount,
    hold_amount = hold_amount + NEW.amount
WHERE id = NEW.account_id;
```

Что это значит для двух операций:

- **Блокировка (amount > 0):** свободный баланс (`balance`) уменьшается, замороженный (`hold_amount`) увеличивается.
- **Разблокировка (amount < 0):** свободный баланс увеличивается, замороженный уменьшается.

Инвариант: `balance + hold_amount` остаётся постоянным. Средства не покидают счёт — они перемещаются между «свободным» и «замороженным» балансом.

### Пример: шаг за шагом

Допустим, на счёте `balance = 100`, `hold_amount = 0`:

| Операция | balance | hold_amount | Сумма (инвариант) |
|-|-|-|-|
...
Holds и transfers — **независимые механизмы**, но они взаимодействуют через `balance` счёта.

### Как hold ограничивает трансферы

Трансфер списывает из `balance` (свободный баланс). Hold уменьшает `balance`, перемещая часть средств в `hold_amount`. Поэтому:

- Для счёта с `kind = 'positive'` (баланс ≥ 0): hold уменьшает доступный для списания баланс. Трансфер, который опустил бы `balance` ниже нуля, будет отклонён CHECK-ограничением.
- Для счёта с `kind = 'any'`: hold уменьшает `balance`, но баланс может уходить в минус, поэтому трансферы не блокируются.
- Для счёта с `kind = 'negative'` (баланс ≤ 0): hold увеличивает замороженный баланс, но списание (уменьшение `balance`) допустимо по ограничению типа.
...
| Влияет на | `balance` и `hold_amount` | Триггер `process_account_transfer` проверяет `locked_at IS NOT NULL` |
| Трансферы | Косвенно ограничивает (через уменьшение `balance`) | Прямо запрещает все исходящие |
| Входящие трансферы | Не влияет | Не влияет |

### Типовой сценарий: авторизация платежа

```
1. Hold $60       → balance: 100→40, hold_amount: 0→60
2. Transfer $30   → balance: 40→10 (списание из свободного баланса)
3. Release $60    → balance: 10→70, hold_amount: 60→0
```

Hold не запрещает трансферы — он лишь уменьшает свободный баланс. Трансферы и holds работают с одним и тем же `balance`.
